...
Por isso, quando existe corrupção, não basta apenas tentar reparar o banco

Também é necessário investigar a causa raiz do problema
...
Podem se passar dias, meses ou até anos sem que a página corrompida seja acessada

A corrupção pode ser percebida somente quando:
...
O tail-log backup tenta capturar os registros do transaction log que ainda não foram copiados em backup

Isso ajuda a reduzir perda de dados e manter a sequência da cadeia de logs
...
O próprio nome da opção já deixa claro o risco

`ALLOW_DATA_LOSS` significa permitir perda de dados
...
A página IAM controla informações de alocação
